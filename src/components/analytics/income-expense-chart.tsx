...
function MetricBar({ label, amount, max, color, percent }: { label: string; amount: number; max: number; color: string; percent?: string }) {
  const width = Math.min((amount / max) * 100, 100)
  return (
    <div className="space-y-2">
      <div className="flex justify-between items-center text-xs">
        <span className="font-bold text-on-surface/70 uppercase tracking-wider text-[10px]">{label}</span>
        <div className="flex items-center gap-2">
          <span className="font-extrabold text-primary">₹{amount.toLocaleString('en-IN')}</span>
          {percent && <span className="text-[10px] font-bold text-on-surface/50">{percent}</span>}
        </div>
      </div>
...
export function IncomeExpenseChart({ current, momChange, className }: IncomeExpenseChartProps) {
  ...
  const expensePercent = `${Math.round((current.totalExpenses / current.income) * 100)}% of income`

  return (
    <div className={cn('space-y-5', className)}>
      <div className="flex items-center justify-between">
        <h4 className="font-serif text-base font-bold text-primary">Income vs Expenses</h4>
        <TrendIndicator value={momChange} label="vs May" invertColors size="sm" />
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 pt-2 border-t border-primary/5">
        {[
          ...
          <div key={item.label} className="text-center">
            ...
            <p className="text-[10px] font-bold text-on-surface/50 uppercase tracking-wider">{item.label}</p>
          </div>
        ))}
      </div>
    </div>
