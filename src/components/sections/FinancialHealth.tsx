...
import { useHealthScore, useInsights } from '@/stores/analytics-store'
...
export default function FinancialHealth({ embedded = false }: { embedded?: boolean }) {
  ...
  const incomeMoM = Math.round(((currentSnapshot.income - previousSnapshot.income) / previousSnapshot.income) * 100)
  const expenseMoM = momChange
  const savingsMoM = Math.round(((currentSnapshot.savingsAmount - previousSnapshot.savingsAmount) / previousSnapshot.savingsAmount) * 100)
    ...
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card variant="default" padding="md">
          <p className="text-[10px] font-bold uppercase tracking-wider text-on-surface/50 mb-2">Monthly Income</p>
          <p className="font-serif text-2xl font-extrabold text-primary mb-2">
            ₹{currentSnapshot.income.toLocaleString('en-IN')}
          </p>
          <TrendIndicator value={incomeMoM} label="vs May" size="sm" />
        </Card>

        {/* Monthly Expenses */}
        <Card variant="default" padding="md">
          <p className="text-[10px] font-bold uppercase tracking-wider text-on-surface/50 mb-2">Monthly Expenses</p>
          <p className="font-serif text-2xl font-extrabold text-primary mb-2">
            ₹{currentSnapshot.totalExpenses.toLocaleString('en-IN')}
          </p>
          <TrendIndicator value={expenseMoM} label="vs May" invertColors size="sm" />
        </Card>

        {/* Net Savings */}
        <Card variant="default" padding="md">
          <p className="text-[10px] font-bold uppercase tracking-wider text-on-surface/50 mb-2">Net Savings</p>
          <p className="font-serif text-2xl font-extrabold text-primary mb-2">
            ₹{currentSnapshot.savingsAmount.toLocaleString('en-IN')}
          </p>
          <TrendIndicator value={savingsMoM} label="vs May" size="sm" />
        </Card>

        {/* Savings Rate */}
        <Card variant="default" padding="md">
          <p className="text-[10px] font-bold uppercase tracking-wider text-on-surface/50 mb-2">Savings Rate</p>
          <p className="font-serif text-2xl font-extrabold text-primary mb-2">
            {currentSnapshot.savingsRate.toFixed(1)}%
          </p>
          <p className="text-[10px] font-bold text-on-surface/40 uppercase tracking-wider">Target: 30%</p>
        </Card>
      </div>
